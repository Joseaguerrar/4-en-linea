...
wxBEGIN_EVENT_TABLE(TableroFrame, wxFrame) EVT_PAINT(TableroFrame::OnPaint)
    wxEND_EVENT_TABLE()
        ...
        TableroFrame::TableroFrame(const wxString& title, int filas,
                                   int columnas)
...
void TableroFrame::OnMouseEvent(wxMouseEvent& event) {
  // Se toma la posicion del mouse en ese momento, mas especificamente la del
  // eje x
  ...
  // Para saber que columna se selecciono, se divide la posicion x del mouse
  // entre el largo de la celda
  ...
  for (int i = 0; i < filas; i++) {
    for (int j = 0; j < columnas; j++) {
      ...
      if (tablero.getTablero()[i][j] == Ficha::Rojo) {
        ...
        dc.DrawCircle(x + celdaLargo / 2, y + celdaAlto / 2,
                      min(celdaLargo, celdaAlto) / 2 - 2);
...
void TableroFrame::OnExit(wxCommandEvent& event) { Close(true); }