...
int jugadorIAInteligente::seleccionarColumna(Tablero& tableroActual) {
  ...
  return minimax(tableroActual, profundidad,menosInfinito,masInfinito,true).second;
    ...
    //si solo queda una columna no tiene sentido hacer el minimax, aparte que se evitar errores de segmentation fault
...
//la función retorna dos valores enteros, la puntuación(se usa dentro del minimax) y la mejor columna (se usa en seleccionar columna) respectivamente
pair<int, int> jugadorIAInteligente::minimax(Tablero& tableroActual,
                                             int profundidad, int alfa, int beta, bool max) {
  // Inicializamos a un valor no válido para asegurarlos que la columna está cambiando
  int mejorColumna = -1;  
  //si la profundidad es 0 o en el tablero se comprueba un ganador se retorna el valor dado por la función heuristica
  ...
  if (max) {
    ...
    for (int j = 0; j < tableroActual.getColumnas(); j++) {
      ...
      if (tableroActual.puedeTirar(j)) {
        //se crea una copia del tablero(importante, esta no puede ser un puntero, porque se estaría modificando el tablero enviado)
        ...
        //si el valor actual supera al valor mínimo se intercambian y se selecciona "la mejor columna en ese momento"
    ...
    for (int j = 0; j < tableroActual.getColumnas(); j++) {
      ...
      if (tableroActual.puedeTirar(j)) {
        ...
        //se llama recursivamente al minimax igual que en el caso anterior, en este caso se llama al primer elemento que es la puntuación
        int valorActual = minimax(tableroCopia, profundidad - 1, alfa, beta, true).first;
        //si el valorActual es menor que el valor se intercambian y se selecciona la mejor columna de ese momento
        if (valorActual < valor) {
          valor = valorActual;
          mejorColumna = j;
        }
...
int jugadorIAInteligente::funcionHeuristica(Tablero& tableroCopia) {
  ...
  //si no se cumple lo anterior se retorna una resta entre la cantidad de lineas ganadoras cuando es max y cuando es min lineasGanadorasMax - lineasGanadorasMin
  return determinarLineasGanadoras(tableroCopia, true) - determinarLineasGanadoras(tableroCopia, false);
}

//esta función acarrea la misma lógica que la de tablero sin embargo en este caso necesitamos valores diferentes
int jugadorIAInteligente::determinarLineasGanadoras(Tablero& tableroCopia, bool isMax){
  //se verifica si es max o min, si isMax es verdadero se usa la ficha con el color de FichaMax y viceversa
  Ficha ficha;
  if(isMax){
    ficha = colorFichaMax;
  }else{
  ...
  //se inicializan valores necesarios para la función lineasGanadoras, filas y columnas del tablero
